...
class SeoAwareItem
{
    ...
    public function __construct(SeoAwareInterface $instance)
    {
        ...
        $type = null;
        if (is_subclass_of($instance, SeoAwarePageObject::class)) $type = self::TYPE_PAGE;
        if (is_subclass_of($instance, SeoAwareListViewObject::class)) $type = self::TYPE_LIST_VIEW;
        if (InstanceUses::check($instance, SeoAwareEntityTrait::class)) $type = self::TYPE_ENTITY;
